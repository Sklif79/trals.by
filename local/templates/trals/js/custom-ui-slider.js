$(function () {
    ...
    $('input[type=text], textarea, select').live('focus', function () {
        ...
        $(this).blur(function () {
            $(this).closest('.g-form__inputwrap').removeClass('cr-state-focus');
        })
    });
...
$(function () {
    ...
    $(".j-slider_range").each(function () {
        ...
        $slider.slider({
            ...
            values: [$from.getFlN(), max_value], //установим ползунки в первоначальные положения (в соот-ии со значениями, введенными в поля)
            slide: function (event, ui) {
                //пользователь начал двигает ползунок - меняем зн-е в соотв. поле
                if (ui.values[0] == ui.value) {
                    $from.val(humanNumber(ui.value, 'down'));
                    $('#smartfilter .submit').trigger('click');
                }
                else {
